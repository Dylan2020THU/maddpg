...
# Replay Buffer
class ReplayMemory:
    def __init__(self, capacity, state_dim, action_dim, num_actions, num_agents):  # 构造函数
        self.capacity = capacity
        self.count = 0
        self.num_actions = num_actions
        self.num_agents = num_agents

        self.state_capacity = np.empty((self.capacity, state_dim))
        self.action_capacity = np.empty((self.capacity, action_dim))
        self.reward_capacity = np.empty((self.capacity, num_agents))
        self.done_capacity = np.empty((self.capacity, num_agents))
        # TODO
